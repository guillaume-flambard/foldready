...
enum HTMLReport {
    ...
    static func render(_ result: AuditResult) -> String {
        ...
        <h2>Potential blockers: confirm build configuration</h2>
        <div class="blockers">
        ...
        <div class="wrap">
          ...
          <footer>FoldReady source assessment. Every check cites Apple guidance. Scores and effort estimates are heuristics, not device certification or a fixed quote.</footer>
        </div>
        </body>
        </html>
        """
    ...
    /// Advisory Duo surface signals: questions, not defects. They never contribute to the
    /// score, and each row carries the runtime check that would settle it.
    private static func advisorySection(_ result: AuditResult) -> String {
        guard !result.advisory.isEmpty else { return "" }
        let rows = result.advisory.map { a -> String in
            let more = a.collapsed > 1 ? " (+\(a.collapsed - 1) more in this file)" : ""
            return "<tr><td>\(esc(a.surface))</td><td>\(esc(a.message))<br><code>\(esc(a.file)):\(a.line)</code>\(more) <a class=\"ref\" href=\"\(esc(a.reference))\">\(esc(sourceLabel(a.reference)))</a></td><td>\(esc(a.runtimeCheck))</td></tr>"
        }.joined(separator: "\n")
        return """
        <h2>Questions for the Duo surfaces</h2>
        <p class="prov">Questions from the source, not defects. They do not affect the score; each one names the runtime check that would settle it.</p>
        <table><thead><tr><th>Surface</th><th>Source signal</th><th>Runtime check that settles it</th></tr></thead><tbody>
        \(rows)
        </tbody></table>
        """
    }
    ...
    private static func blockerRow(_ b: Blocker) -> String {
        ...
        <div class="blocker\(b.stopsLaunch ? "" : " optout")">
          <div class="tag">\(b.stopsLaunch ? "SDK-CONDITIONAL SIGNAL" : "OPT-OUT SIGNAL")</div>
    ...
    private static func outcomeRow(_ o: CheckOutcome) -> String {
        ...
        // The Apple source sits next to the check, so a disputed score is an argument
        // about the code rather than about FoldReady's authority.
        return """
        <div class="bar">
          <div class="name">\(esc(o.title))<br><a class="ref" href="\(esc(o.reference))">\(esc(sourceLabel(o.reference)))</a></div>
    ...
    /// Short human label for an Apple reference URL, e.g. "developer.apple.com · WWDC26 278".
    private static func sourceLabel(_ url: String) -> String {
        let tail = url
            .replacingOccurrences(of: "https://developer.apple.com/", with: "")
            .trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        if tail.hasPrefix("videos/play/") {
            let parts = tail.split(separator: "/")
            if parts.count >= 4 { return "Apple · \(parts[2].uppercased()) session \(parts[3])" }
        }
