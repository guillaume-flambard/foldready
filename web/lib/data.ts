...
export const APPS: AppScore[] = [
  { name: "MochiDiffusion", slug: "mochidiffusion", score: 92, grade: "A", risk: "low", hours: 27.5, swiftFiles: 58, findings: 0, repo: "MochiDiffusion/MochiDiffusion", checks: { adaptive: 93, parallel: 100, nav: 100, scene: 100, fold: 70, state: 70, framework: 100 } },
  { name: "IceCubesApp", slug: "icecubesapp", score: 72, grade: "B", risk: "low", hours: 215.5, swiftFiles: 428, findings: 37, repo: "Dimillian/IceCubesApp", checks: { adaptive: 93, parallel: 100, nav: 40, scene: 100, fold: 30, state: 70, framework: 91 } },
  ...
  { name: "Dime", slug: "dime", score: 56, grade: "C", risk: "medium", hours: 80, swiftFiles: 94, findings: 38, repo: "rarfell/dimeApp", checks: { adaptive: 0, parallel: 100, nav: 40, scene: 100, fold: 70, state: 70, framework: 90 } },
...
const ICECUBES: ReportDetail = {
  ...
  breakdownNotes: {
    ...
    parallel: "Resizable presentation not blocked · w 0.10",
  ...
  roadmap: [
    ...
    { title: "Resizable presentation verification", body: "Capture inner-display layouts at 7.8in on a device and confirm the opt-in holds for every tab.", hours: 8, pct: 0.08 },
...
function genericReport(app: AppScore): ReportDetail {
  ...
  if (app.checks.fold < 70) findings.push({ severity: "Minor", check: "Fold state", message: "No adaptive geometry handling: layout does not branch on the size class or the scene\u2019s effective geometry.", file: "app-wide" });
  ...
  const roadmap: RoadmapStep[] = [
    { title: "Resizable presentation verification", body: "Capture inner-display layouts and confirm the opt-in holds.", hours: 8, pct: 0.08 },
  ...
  return {
    ...
    breakdownNotes: {
      ...
      parallel: app.checks.parallel === 100 ? "resizable presentation not blocked" : "verify opt-in",
