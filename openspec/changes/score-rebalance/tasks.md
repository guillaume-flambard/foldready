...
- [x] 5.4 Update `web/lib/data.ts`, the report pages and the index copy to the new scores
      and the blocker verdict. The index is now generated by `Scripts/generate-index.py`
      from real result files rather than hand-edited, and covers all twenty corpus apps.
