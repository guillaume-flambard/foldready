...
private func result(total: Double, checks: [(String, Double)]) -> AuditResult {
    let outcomes = checks.map { key, score in
        CheckOutcome(key: key, title: key, weight: 1.0 / Double(checks.count),
            score: score / 100, detail: "", reference: Reference.modernizeUIKit,
    ...
    return AuditResult(root: "/tmp/app", appName: "App", generatedAt: Date(),
        ...
        findings: Finding.deterministicOrder(outcomes.flatMap(\.findings)),
        stats: AuditStats(swiftFiles: 3, swiftuiFiles: 2, uikitFiles: 1,
            xibOrStoryboard: 0, infoPlists: 1),
        hoursEstimate: 4)
}
